function validateInputTextEmpty(inputTexts) {
    ...
    for (let index = 0; index < inputTexts.length; index += 1) {
        if (
            ...
            window.getComputedStyle(inputTexts[index]).display !== "none"
        ) {
            anyEmpty = true;
            break;
        }
...
function changeRightContent() {
    ...
    document.getElementById("info-user").innerHTML = `E-mail ou Telefone: ${
    document.getElementById("phone_email").value
  } <br> Data de Nacimento: ${
    document.getElementById("birthdate").value
  } <br> Gênero: ${genderValue}`;
...
function stopDefAction(evt) {
    ...
    validateTextsOK = !validateInputTextEmpty(
        document.querySelectorAll(
            ".facebook-signup input[type=text], input[type=password]"
        )
    );
    validateRadiosOK = !validateInputRadioUnchecked(
        document.querySelectorAll(".facebook-signup input[type=radio]")
    );
...
window.onload = function() {
    ...
    const inputsRadio = document.querySelectorAll(
        ".facebook-signup input[type=radio]"
    );
